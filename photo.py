...
def main():
    ...
    menu = ['Filters','Image Corrections','Informations']
    ...
    if op == 'Filters':
        ...
        if filters == 'Grayscale':
            st.write(str(type(image)))
            img_convert = np.asarray(image)
            
            gray_image = cv2.cvtColor(img_convert,cv2.COLOR_RGB2GRAY)
            st.write(str(type(img_convert)))
    ...
    if op == 'Image Corrections':
        ...
        if MImage == 'Contrast':
            ...
            slide = st.sidebar.slider('Brightness',0.0,2.0,1.0)
